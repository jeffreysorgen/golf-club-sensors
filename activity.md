...
- There may be a way to use `if(serial) {` in the code so that we do not need to modify it every time we want to test the battery-only mode. Could explore this, but tabling it for the future.
